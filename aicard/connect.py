"""Native cognis-connect emit for aicard — forward findings to any platform.

Maps aicard's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    aicard ... --format json | aicard-emit --to stix
    aicard-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['severity'] = rec.get('risk', 'info')
